...
- **The one sanctioned browser-direct path** is `src/services/ai/client/openaiCompat.ts` — the `local` / `network` generation providers, which call the user's own OpenAI-compatible server with no Worker, no `x-ptscribe-key`, and `credentials: 'omit'`. The rule above protects a provider credential; there is none here, and proxying the transcript through Cloudflare to reach a box on the clinician's desk would put PHI on our infrastructure for nothing. Signed-in accounts only (`useSelfHostedAllowed`); a self-hosted failure never auto-falls-back to cloud. See [ADR-0011](adr/0011-direct-browser-calls-to-self-hosted-llm-endpoints.md).
...
- The visibility handler also emits a `{type: 'backgrounded'}` event on `subscribeEvents` the first time the tab is hidden during a clip (guarded by a one-shot ref, not re-emitted on repeat hide/show cycles within the same clip). `useCapturePhase` owns the derived `backgrounded` boolean that `useSessionMachine`'s `showBackgroundWarning` selector reads to surface a "verify duration" warning. The guard is reset only by the next `start()` or `reset()` — do not clear it on `visibilitychange → visible`.
- `useRecorder` exposes stop reasons and advisories (soft-warn, silence start/end, backgrounded) through `subscribeEvents(cb)`, a multi-subscriber stream (same `Set<callback>` shape as `subscribeDuration`) rather than boolean fields — several of these can be true simultaneously (e.g. backgrounded _and_ hard-capped), which a single snapshot field can't represent. Callers must subscribe before the event fires; nothing is replayed or coalesced.
