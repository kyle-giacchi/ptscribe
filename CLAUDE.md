...
- **AI calls go through our Worker proxy — except self-hosted endpoints.** Whisper and Anthropic are reached via `POST /api/transcribe` and `POST /api/generate` on our Cloudflare Worker; provider credentials are server-side secrets the browser never sees. Requests carry `sha256(PTSCRIBE_GATE)` in `x-ptscribe-key` (obscurity gate — abuse caps live server-side). Settings still surfaces a HIPAA disclaimer because data still leaves the device. **The one exception:** the `local` / `network` generation providers call the user's own OpenAI-compatible server directly from the browser — no Worker, no gate header, `credentials: 'omit'`. There is no credential to protect, and proxying PHI through our infrastructure would defeat the point. See [ADR-0011](docs/adr/0011-direct-browser-calls-to-self-hosted-llm-endpoints.md).
